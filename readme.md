## Laravel Translation Manager - Personal fork

This package is a fork of barryvdh's Translation manager (forked at 2018/01/22) made in order to implement and adapt it for my own projects.

It going to be developed, tested and used over laravel 5.5.*. Contributors are welcome to address any issue as well to help to resolve it.
...
Require this package in your composer.json and run composer update (or run `composer require barryvdh/laravel-translation-manager` directly):
    ...
    "JoaoRivera/laravel-translation-manager": "0.2.x@dev"

After updating composer, add the ServiceProvider to the providers array in config/app.php

    'JoaoRivera\TranslationManager\ManagerServiceProvider',

You need to run the migrations for this package.

    $ php artisan vendor:publish --provider="JoaoRivera\TranslationManager\ManagerServiceProvider" --tag=migrations
    $ php artisan migrate
...
You need to publish the config file for this package. This will add the file `config/translation-manager.php`, where you can configure this package.
    ...
    $ php artisan vendor:publish --provider="JoaoRivera\TranslationManager\ManagerServiceProvider" --tag=config
...
In order to edit the default template, the views must be published as well. The views will then be placed in `resources/views/vendor/translation-manager`.
    ...
    $ php artisan vendor:publish --provider="JoaoRivera\TranslationManager\ManagerServiceProvider" --tag=views
...
In your config/app.php, comment out the original TranslationServiceProvider and add the one from this package:
    ...
    'JoaoRivera\TranslationManager\TranslationServiceProvider',
